class TopDownSolution:
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        if len(s1) + len(s2) != len(s3):
            return False
        
        cache = {}
        def dfs(i, j):
            if i == len(s1) and j == len(s2):
                return True
            if (i, j) in cache:
                return cache[(i, j)]
            
            if i < len(s1) and s1[i] == s3[i + j] and dfs(i + 1, j):
                return True
            if j < len(s2) and s2[j] == s3[i + j] and dfs(i, j + 1):
                return True

            cache[(i, j)] = False
            return False

        return dfs(0, 0)

#Test Case
my_topDown_solution = TopDownSolution()
print("Test Case 1: ", my_topDown_solution.isInterleave("aabcc", "dbbca", "aadbbcbcac"))
print("Test Case 2: ", my_topDown_solution.isInterleave("aabcc", "dbbca", "aadbbbaccc"))
print("Test Case 3: ", my_topDown_solution.isInterleave("", "", ""))

class BottomUpSolution:
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        dp = [[False for _ in range(len(s2) + 1)] for _ in range(len(s1) + 1)]
        dp[len(s1)][len(s2)] = True

        for i in range(len(s1), -1, -1):
            for j in range(len(s2), -1, -1):
                if i < len(s1) and s1[i] == s3[i + j] and dp[i + 1][j]:
                    dp[i][j] = True
                if j < len(s2) and s2[j] == s3[i + j] and dp[i][j + 1]:
                    dp[i][j] = True
        return dp[0][0]


#Test Case
my_bottomUp_solution = BottomUpSolution()
print("Test Case 1: ", my_bottomUp_solution.isInterleave("aabcc", "dbbca", "aadbbcbcac"))
print("Test Case 2: ", my_bottomUp_solution.isInterleave("aabcc", "dbbca", "aadbbbaccc"))
print("Test Case 3: ", my_bottomUp_solution.isInterleave("", "", ""))






